...
function formatTelegramMessage(
  ...
  return (
    `${isUpdate ? "📝" : "🎉"} ${isUpdate ? "Update van wachtlijst aanmelding" : "Nieuwe aanmelding voor de wachtlijst"}!\n\n` +
...
export async function submitToWaitlist(formData: Record<string, unknown>) {
            ...
            text: {
              content:
                String(formData.companyName || "")
                  .charAt(0)
                  .toUpperCase() + String(formData.companyName || "").slice(1),
            },
          },
        ],
      },
  ...
  const telegramResponse = await bot.sendMessage(TELEGRAM_CHAT_ID, message, {
    parse_mode: "Markdown",
  });
...
export async function updateWaitlistSubmission(
  ...
  const existingMessageId = parseInt(
    notionPage.properties.telegramMessageId.rich_text?.[0]?.text?.content
  );

  console.log(
    "existingMessageId",
    existingMessageId,
    notionPage.properties.Email.email,
    updatedMetadata
  );
